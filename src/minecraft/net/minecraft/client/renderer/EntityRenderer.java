...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && Minecraft.theWorld != null)
        {
            ...
            Vec3 vec33 = null;
            float f = 1.0F;
            List list = Minecraft.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(vec31.xCoord * d0, vec31.yCoord * d0, vec31.zCoord * d0).expand(f, f, f), Predicates.and(EntitySelectors.NOT_SPECTATING, new EntityRenderer1(this)));
            double d2 = d1;
        ...
        else if (this.mc.gameSettings.thirdPersonView > 0)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    f4 = f4 * 0.1F;
                    f5 = f5 * 0.1F;
                    MovingObjectPosition movingobjectposition = Minecraft.theWorld.rayTraceBlocks(new Vec3(d0 + (double)f3, d1 + (double)f4, d2 + (double)f5), new Vec3(d0 - d4 + (double)f3 + (double)f5, d1 - d6 + (double)f4, d2 - d5 + (double)f5));
        ...
        else
        {
            ...
            if (flag && !((EntityPlayer)entity).capabilities.allowEdit)
            {
                ...
                if (this.mc.objectMouseOver != null && this.mc.objectMouseOver.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK)
                {
                    ...
                    {
                        flag = ReflectorForge.blockHasTileEntity(iblockstate) && Minecraft.theWorld.getTileEntity(blockpos) instanceof IInventory;
                    }
                    else
                    {
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        Main.getInstance().getEventBus().post(event);
        GL11.glPopAttrib();
        ...
        if (!flag2 && this.renderHand && !Shaders.isShadowPass)
        {
            ...
            {
                if(ModuleManager.getModule(Chams.class).toggled && mc.thePlayer.getHeldItem() == null && Chams.hand.isChecked()){
        ...
        if (f != 0.0F && Config.isRainSplash())
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                if (blockpos1.getY() <= blockpos.getY() + b0 && blockpos1.getY() >= blockpos.getY() - b0 && biomegenbase.canSpawnLightningBolt() && biomegenbase.getFloatTemperature(blockpos1) >= 0.15F)
                {
                    ...
                    {
                        Minecraft.theWorld.spawnParticle(EnumParticleTypes.SMOKE_NORMAL, (double)blockpos1.getX() + d3, (double)((float)blockpos1.getY() + 0.1F) - block.getBlockBoundsMinY(), (double)blockpos1.getZ() + d4, 0.0D, 0.0D, 0.0D);
                    }
                    else if (block.getMaterial() != Material.air)
                    {
                        ...
                        Minecraft.theWorld.spawnParticle(EnumParticleTypes.WATER_DROP, (double)blockpos2.getX() + d3, (double)((float)blockpos2.getY() + 0.1F) + block.getBlockBoundsMaxY(), (double)blockpos2.getZ() + d4, 0.0D, 0.0D, 0.0D);
                    }
                }
            }
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        else if (block.getMaterial() == Material.water)
        {
            ...
            Vec3 vec34 = CustomColors.getUnderwaterColor(Minecraft.theWorld, this.mc.getRenderViewEntity().posX, this.mc.getRenderViewEntity().posY + 1.0D, this.mc.getRenderViewEntity().posZ);

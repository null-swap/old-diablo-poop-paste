...
public class RenderUtils {
    ...
    public static void drawGlow(double x, double y, double size, int col){
        for(int i = 0; i < 360 ; i++){
            GlStateManager.pushMatrix();
            GlStateManager.translate(x,y,0);
            glRotated(i,0,0,1);
            Color color = new Color(col);
            drawGradientRect(0,0,size, 0 + size, ColorHelper.getColor(color.getRed(),color.getGreen(),color.getBlue(),0), ColorHelper.getColor(color.getRed(),color.getGreen(),color.getBlue(),10));
            GlStateManager.translate(-x,-y,0);
            glRotated(-i,0,0,1);
    ...
    public static void drawImage(int i, int i1, int i2, int i3, ResourceLocation rec) {
        GlStateManager.pushMatrix();
        Minecraft.getMinecraft().getTextureManager().bindTexture(rec);
        GlStateManager.color(1,1,1);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
        Gui.drawModalRectWithCustomSizedTexture(i,i1,0,0,i2,i3,i2,i3);
        GlStateManager.popMatrix();
    }
    public static void drawImage(int i, int i1, int i2, int i3, ResourceLocation rec, int opacity) {
        GlStateManager.pushMatrix();
    ...
    public static void drawLineToPosition(BlockPos blockPos, int color) {
        ...
        double renderPosXDelta = blockPos.getX() - mc.getRenderManager().viewerPosX + 0.5D;
        double renderPosYDelta = blockPos.getY() - mc.getRenderManager().viewerPosY + 0.5D;
        double renderPosZDelta = blockPos.getZ() - mc.getRenderManager().viewerPosZ + 0.5D;
        GL11.glPushMatrix();
        GL11.glEnable(3042);
        GL11.glEnable(2848);
        GL11.glDisable(2929);
